"use client";
import { ChevronsLeft, Menu, MenuIcon, PlusCircle, Search } from "lucide-react";
...
const Navigation = () => {
  ...
  const sidebarRef = useRef<ElementRef<"aside">>(null); // Nice ref example
  ...
  const handleMouseUp = () => {
    ...
    document.removeEventListener("mouseup", handleMouseUp); // Nice implementation of sidebar adjustability
  };
  ...
  const resetWidth = () => {
    if (sidebarRef.current && navbarRef.current) {
      ...
      navbarRef.current.style.setProperty(
        "width",
        isMobile ? "0" : "calc(100% - 240px)" // calculated value example
      );
  ...
  const handleCreate = () => {
    ...
    toast.promise(promise, {
      ...
      error: "Failed to create a new note.", // Nice toast example
    });
  };
